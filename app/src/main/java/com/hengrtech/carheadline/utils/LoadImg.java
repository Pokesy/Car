...
public class LoadImg {
    ...
    public Bitmap loadImage(final ImageView imageView, final String imageUrl,
        ...
        final String filename = (ss.substring(ss.lastIndexOf("/", ss.lastIndexOf("/") - 1) + 1, ss.length())).replace("/","");
        // 图片保存到本地时的地址
        ...
        if (imageUrl != null && !imageUrl.equals("")) {
            ...
            Thread thread = new Thread() {
                public void run() {
                    // 网络下载时的字节流
                    HttpEntity httpEntity = DownBitmap.getInstance().getInputStream(imageUrl);
                    if (httpEntity != null) {
                        ...
                        Log.e("图片字节流长度", httpEntity.getContentLength() + "");
                        if (httpEntity.getContentLength() >= 1000 * 1024) {
                            op.inSampleSize = 4;
                        } else if (httpEntity.getContentLength() >= 500 * 1024) {
                            op.inSampleSize = 2;
                        } else {
                            op.inSampleSize = 0;
                        }
    ...
    private String devide(String imageUrl) {
        String s=null;
